...
uint8_t mpu_write_byte(uint8_t const reg, uint8_t const data) {
    tx = data;
    HAL_I2C_Mem_Write(&MPU_HI2C, MPU_I2C_ADDR, reg & 0x7F, I2C_MEMADD_SIZE_8BIT, &tx, 1, 1000);
...
uint8_t mpu_read_byte(uint8_t const reg) {
    HAL_I2C_Mem_Read(&MPU_HI2C, MPU_I2C_ADDR, reg | 0x80, I2C_MEMADD_SIZE_8BIT, &rx, 1, 1000);
...
uint8_t mpu_read_bytes(uint8_t const regAddr, uint8_t *pData, uint8_t len) {
    HAL_I2C_Mem_Read(&MPU_HI2C, MPU_I2C_ADDR, regAddr | 0x80, I2C_MEMADD_SIZE_8BIT, pData, len, len * 700);
...
/*void MPU6050ReturnData(int16_t *AngularVelocity, int16_t *Angular)
{
    mpu_get_data();
    imu_ahrs_update();
    imu_attitude_update();

    AngularVelocity[0] = mpu_data.gx;//加速度
    AngularVelocity[1] = mpu_data.gy;
    AngularVelocity[2] = mpu_data.gz;

    Angular[0] = imu.rol;//角度
    Angular[1] = imu.pit;
    Angular[2] = imu.yaw;

//    Angular[0] = mpu_data.ax;
//    Angular[1] = mpu_data.ay;
//    Angular[2] = mpu_data.az;
}
*/
