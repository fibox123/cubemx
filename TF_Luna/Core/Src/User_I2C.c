...
{    
		GPIO_InitTypeDef GPIO_InitStruct = {0};  
    GPIO_InitStruct.Pin = USER_I2C_SDA_PIN;    
    GPIO_InitStruct.Mode = (SDAMode == I2C_SDA_IN) ? GPIO_Mode_IPU : (MODE_OUTPUT | OUTPUT_PP);    
		HAL_GPIO_Init(GPIOF, &GPIO_InitStruct); 
}
...
static __I2C_ACK_TypeDef I2C_RecvAck(uint32_t Timeout)
{
	...
	while(1)
	{
		ackStatus = (I2C_SDA_IN() == GPIO_PIN_RESET) ? I2C_ACK : I2C_NACK;
...
{
    uint32_t i;
    uint32_t data = 0, GPIO_PINStatus = 0;
    ...
    {  
        ...
        GPIO_PINStatus = (I2C_SDA_IN() == GPIO_PIN_SET) ? 1 : 0;
        data |= GPIO_PINStatus;     
